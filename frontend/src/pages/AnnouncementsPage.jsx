      ...
      <header className="sticky top-0 z-50 w-full border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-950/80 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto h-16 px-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link
              to={getDashboardLink()}
              className="px-4 py-2 bg-slate-200 dark:bg-slate-800 text-sm font-bold rounded-lg hover:bg-slate-300 dark:hover:bg-slate-700 transition-colors"
            >
              Back to Dashboard
            </Link>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="p-16 text-center bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
              <div className="w-16 h-16 bg-slate-100 dark:bg-slate-800 rounded-full flex items-center justify-center mx-auto mb-4">
                ...
                >
                  <path
                    ...
                    strokeWidth="2"
                    d="M11 5.882V19.24M11 5.882a1.5 1.5 0 013 0v1.5a1.5 1.5 0 01-3 0m-3 6a1.5 1.5 0 00-3 0v1.5a1.5 1.5 0 003 0m-3-6a1.5 1.5 0 013 0v1.5a1.5 1.5 0 01-3 0m6 9a1.5 1.5 0 00-3 0v1.5a1.5 1.5 0 003 0m-3-6a1.5 1.5 0 013 0v1.5a1.5 1.5 0 01-3 0"
                  />
                </svg>
              </div>
              ...
              <p className="text-slate-500 text-sm mt-1">
                No new announcements have been posted for you.
              </p>
            </div>
          ) : (
            announcements.map((notice) => (
              <div
                key={notice._id}
                className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col md:flex-row hover:shadow-md transition-shadow relative"
              >
                <div
                  className={`p-4 flex flex-col items-center justify-center shrink-0 w-full md:w-36 ${
                ...
                >
                  ...
                  <span className="text-[10px] mt-2 text-center font-medium opacity-80">
                ...
                <div className="p-6 md:p-8 w-full">
                  ...
                  <p className="text-slate-600 dark:text-slate-300 mb-6 whitespace-pre-wrap leading-relaxed">
                    {notice.content}
                  </p>
                  ...
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 pt-4 border-t border-slate-100 dark:border-slate-800">
                      ...
                      <div>
                        <p className="text-xs font-bold text-slate-700 dark:text-slate-300">
                          {notice.admin?.role === 'superadmin'
                            ? 'System Overseer'
                            : `${notice.admin?.firstName} ${notice.admin?.lastName}`}
                        </p>

                        {notice.admin?.role !== 'superadmin' && (
                          <p className="text-[10px] text-slate-500 font-semibold">
                            {notice.admin?.stream} {notice.admin?.branch}
                          </p>
                        )}
                      </div>
                    </div>
                    ...
                    {notice.attachment &&
                      (notice.fileType === 'image' ? (
                        <a
                          href={`${import.meta.env.VITE_API_URL}${notice.attachment}`}
                          target="_blank"
                          rel="noreferrer"
                          className="block max-w-sm mt-4 sm:mt-0"
                        >
                          <img
                            src={`${import.meta.env.VITE_API_URL}${notice.attachment}`}
                            alt="Attachment"
                            className="w-full h-auto max-h-48 object-cover rounded-xl border-2 border-slate-200 dark:border-slate-700 shadow-sm hover:opacity-90 transition-opacity"
                          />
                        </a>
                      ) : (
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="flex items-center gap-2 px-5 py-2.5 bg-blue-50 dark:bg-blue-500/10 text-blue-700 dark:text-blue-400 rounded-xl hover:bg-blue-100 dark:hover:bg-blue-500/20 transition-colors font-bold text-sm shadow-sm border border-blue-100 dark:border-blue-800/50"
                        >
                          <svg
                            className="w-5 h-5"
                            fill="none"

...
export default function StudentDashboard() {
  ...
  const fetchDashboardData = async (token) => {
    setIsLoading(true);
    try {
      const [complaintsRes, managementRes, announcementsRes] = await Promise.all([
        fetch(`${API_BASE}/api/complaints/my-complaints`, { headers: { 'Authorization': `Bearer ${token}` } }),
        fetch(`${API_BASE}/api/auth/admins`, { headers: { 'Authorization': `Bearer ${token}` } }),
                      ...
                      <div key={complaint._id} className="p-6 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                          <div className="flex gap-4">
                            {complaint.image ? (
                              <img src={`${API_BASE}${complaint.image}`} alt="Issue" className="w-16 h-16 rounded-lg object-cover border border-slate-200 dark:border-slate-700 hidden sm:block" />
                            ) : (
                        ...
                        {notice.attachment && (
                          notice.fileType === 'image' ? (
                            <a href={`${API_BASE}${notice.attachment}`} target="_blank" rel="noreferrer" className="block max-w-sm mt-4 sm:mt-0">
                              <img src={`${API_BASE}${notice.attachment}`} alt="Attachment" className="w-full h-auto rounded-xl border-2 border-slate-200 dark:border-slate-700 shadow-md hover:opacity-90 transition-opacity" />
                            </a>
                          ) : (
                            <a href={`${API_BASE}${notice.attachment}`} download={notice.originalFileName} target="_blank" rel="noreferrer" className="flex items-center gap-2 px-4 py-2 bg-red-50 dark:bg-red-500/10 text-red-700 dark:text-red-400 rounded-lg hover:bg-red-100 dark:hover:bg-red-500/20 transition-colors font-bold text-sm">

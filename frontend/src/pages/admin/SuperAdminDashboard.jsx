      ...
      <main className="max-w-7xl mx-auto px-6 py-8 flex flex-col md:flex-row gap-8">
        ...
        <div className="flex-1">
                        ...
                        <div className="flex items-center gap-4">
                          <div>
                            ...
                            <div className="flex flex-col text-sm text-slate-500 gap-1 mt-1">
                              <span><strong className="text-slate-600">Requested Branch: </strong> <span className="text-indigo-600 font-bold bg-indigo-50 px-2 py-0.5 rounded">{admin.stream} {admin.branch} • Sem {admin.semester}</span></span>
                            </div>
                          </div>
                        </div>
                        <div className="flex items-center gap-3">
          ...
          {activeTab === 'manage-admins' && (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="mb-8">
                <h2 className="text-3xl font-black tracking-tight mb-2">Personnel Management</h2>
                <p className="text-slate-500 dark:text-slate-400">View and manage all active administrators on the platform.</p>
              </div>

              <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                {isLoading ? (
                  <div className="p-12 text-center text-slate-500">Loading active personnel...</div>
                ) : activeAdmins.length === 0 ? (
                  <div className="p-16 text-center text-slate-500">There are no approved administrators currently in the system.</div>
                ) : (
                    ...
                    <table className="w-full text-left text-sm">
                      <thead className="bg-slate-50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-800 text-xs uppercase text-slate-500">
                        <tr>
                          ...
                          <th className="px-6 py-4 font-bold">Assigned Department</th>
                      ...
                      <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                        {activeAdmins.map((admin) => (
                          <tr key={admin._id} className="hover:bg-slate-50 dark:hover:bg-slate-800/20">
                            <td className="px-6 py-4">
                              <div className="font-bold text-slate-900 dark:text-white">{admin.firstName} {admin.lastName}</div>
                              <div className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mt-0.5">ID: {admin.identifier}</div>
                            </td>
                            <td className="px-6 py-4">
                              <span className="px-2.5 py-1 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-400 rounded-md text-xs font-bold">
                                {admin.stream} {admin.branch}
                              </span>
                              <div className="text-xs text-slate-500 mt-1.5 font-medium">Semester {admin.semester}</div>
                            </td>
                            <td className="px-6 py-4">
                              <div className="text-xs font-medium text-slate-600 dark:text-slate-300">{admin.email}</div>
                              <div className="text-xs font-medium text-slate-500 mt-1">{admin.contactNumber}</div>
                            </td>
